...
class mpd_node:
    ...
    def get_video_length_seconds(self) -> float:
        timeStr = self.get_period_info()['duration']
        timeList = re.split(r'PT|H|M|S', timeStr)
        seconds = float(timeList[1]) * 60 * 60 + float(timeList[2]) * 60 + float(timeList[3])
        return seconds

    def get_segment_length_seconds(self) -> float:
        timescale = self.get_segment_template()['timescale']
        duration = self.get_segment_template()['duration']
